...
Once the Town/Main Game interface is stable, changes confined to `Town*`, `SlimeFight*` and
their Town tests may use `./check.sh --town -q` (or `-r` for rules only). This runs the Town
suite and selected Town frames. Use `./build.sh --developer --town` for an installable developer
APK with that scoped gate. Changes to the shared interface or other game files use the table above.
For an install after shared interface changes, `./build.sh --developer --rules-checks` folds the
full rules gate into the build without exporting unrelated frames or audio.
...
Local installs target **DDDUMPLING Dev** (`com.dddumpling.game.dev`) and reuse the existing
`build/debug.keystore`. Use the normal developer build/install flow; do not substitute the
Play upload key or uninstall the app to work around an update failure.

An older checkout can produce a version-code downgrade even when its signing certificate
matches the installed Dev app. This caused the code-23 APK versus installed code-24 failure;
it was not a signing mismatch. `build.sh` now keeps the generated local Dev manifest's version
code at least as high as the installed app, without changing tracked release metadata.
If an update still fails, compare the installed and built package, certificate, and version
code before changing signing. See [build troubleshooting](docs/engineering/build.md).
