...
| **boss befriended** | the boss reward screen. The run companion stays opaque as its rounded home fades away, moves to the centered space beneath `A FRIEND RETURNS` or `JOINS YOUR COLLECTION`, sends a heart up to the new boss, then returns as its home fades back in; the old collection lineup is omitted so the two characters own the moment | `BossCollect.celebration` |
