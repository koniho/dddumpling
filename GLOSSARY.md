...
| **high-score screen** | tap BEST on the title to browse the ten highest saved runs; ties favor newer runs, and the latest completed run glows. If it misses the top ten, it appears as an extra row at the bottom; all rows fit on one page. Each compact row shows score, death blurb, overlapping defeated bosses, and a dumpling icon with its reward count including duplicates; tap for completed stages, powerup and rescue-swipe usage, and the full saved summary | `HighScores`, `HighScoreScreen` |
