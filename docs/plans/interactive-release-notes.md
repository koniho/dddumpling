...
The kawaii steamer opens a compact release list for v0.1.19, v0.1.18 and v0.1.17. Each release has left-aligned change icons that wrap to additional rows when needed; all its fixes share one cute bug icon. Only the icons open feature pages; versions and surrounding space are not links. No feature labels on the list. Vertical swipes scroll when the content exceeds the available height, without selecting an icon.
