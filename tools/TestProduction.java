...
final class TestProduction extends Check {
    public static void main(String[] args) throws Exception {
        ...
        check("production retains progression and collections",
                c.collected == 1L && c.steamer.opens == 5 && c.stars.wins == 3);
        c.openSettings();
        ...
        check("settings actions cannot clear collections", c.collected == 1L
                && c.collected == 1L && !c.clearArmed);
        check("policy is accessible on title", PrivacyUi.hit(c, L, L.w - L.unit, L.dangerY - 2f * L.unit));
        c.startGame();
        check("policy does not intercept gameplay", !PrivacyUi.hit(c, L, L.w - L.unit, L.dangerY - 2f * L.unit));
        ...
        check("normal production rewards still work", c.collectTotal == 9
                && (c.collected & (1L << (Collect.BOSS_FIRST + Boss.OCTOPUS))) != 0);
